...
public class CentralizedHeuristic extends Heuristic {
    ...
    public CentralizedHeuristic(State initialState) {
    ...
    public int h(State n) {
        ...
        if (!punishForStandingStillDeactivated) {
    ...
    public int getDistsToBoxesFromAgents(State n) {
        ...
        for (Agent agent : n.agents) {
            ...
            for (int idx = 0; idx < n.boxes.length; idx++) {
                ...
                if (box.color == agent.color && vitalPath.size() > 0 && !unblockingDeactivated) { //is known to block
